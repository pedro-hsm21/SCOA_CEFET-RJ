...
public class TelaDisciplinas extends JFrame {
	...
	public TelaDisciplinas() {
		...
		table.setModel(new DefaultTableModel(new Object[][] { { null, null }, }, new String[] { "ID", "Disciplina" }) {
			/**
			 * 
			 */
			private static final long serialVersionUID = 1L;
		...
		JButton btnNewButton = new JButton("Nova Disciplina");
		...
		btnAlterar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (clicou >= 0) {
					...
					// disciplinas.get(clicou).getNome_usuario() + " na linha " +
					// clicou);
					try {
						TelaCadastroDisciplina tela = new TelaCadastroDisciplina();
						DisciplinaController controllerD = new DisciplinaController();
						Disciplina disciplina = controllerD.buscarDisciplina(disciplinas.get(clicou).getIdDisciplina());
						 //JOptionPane.showMessageDialog(null,"enviou codigo" + disciplina.getId_disciplina() + "e periodo" + disciplina.getPeriodo_disciplina());
						tela.povoarComboBox();
						tela.carregarValores(disciplina);
						tela.carregarTable(disciplina.getIdDisciplina());
						dispose();
						tela.setVisible(true);
		...
		btnExcluir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				int clicou = table.getSelectedRow();
				if (clicou >= 0) {
					int rs = JOptionPane.showConfirmDialog(null, "Excluir " + disciplinas.get(clicou).getNome(),
							"Atenção", JOptionPane.YES_NO_OPTION);
					if (rs == JOptionPane.YES_OPTION) {
						try {
							DisciplinaController controllerD = new DisciplinaController();
							Disciplina disciplina = controllerD.buscarDisciplina(disciplinas.get(clicou).getIdDisciplina());
							boolean status = controllerD.excluirDisciplina(disciplina.getIdDisciplina());
							if (status == true) {
	...
	void carregarTable() {
		...
		try {
			disciplinas = controllerD.listarDisciplinas();
			disciplinas.forEach((Disciplina disciplina) -> {
				tablemodel.addRow(new Object[] { disciplina.getIdDisciplina(), disciplina.getNome() });
			});
